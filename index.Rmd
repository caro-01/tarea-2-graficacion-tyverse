...
```{r Gráfico_ barras_agrupado_covid}
# Generación de un nuevo data frame
...
geom_col(position = "dodge") +
  coord_flip() +
  ggtitle("Fallecidos por Covid-19 en Costa Rica") +
  xlab("Mes") +
...
```{r}
ggplot()+
  geom_col(data = sum(mujeres), aes (x = mes, y = suma_mujeres_fallecidas, color = "blue"))+
  geom_col(data = sum(hombres), aes (x = mes, y = suma_hombres_fallecidos, color = "green"))
```
